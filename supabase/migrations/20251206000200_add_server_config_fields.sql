-- Add server config fields to websites table
ALTER TABLE public.websites
ADD COLUMN IF NOT EXISTS robots_txt text,
ADD COLUMN IF NOT EXISTS htaccess text,
ADD COLUMN IF NOT EXISTS redirect_404 boolean DEFAULT true;
...
BEGIN
  ...
  IF v_existing.uuid IS NULL THEN
    ...
    INSERT INTO websites (
      ...
      robots_txt, htaccess, redirect_404
    )
    VALUES (
      v_uuid,
      v_domain,
      ...
      NULLIF(p_data->>'robots_txt', ''),
      NULLIF(p_data->>'htaccess', ''),
      COALESCE((p_data->>'redirect_404')::boolean, true)
    )
  ...
  ELSE
    UPDATE websites SET
      ...
      robots_txt = COALESCE(
        CASE WHEN p_data ? 'robots_txt' AND p_data->>'robots_txt' IS NOT NULL THEN NULLIF(p_data->>'robots_txt', '') END,
        robots_txt
      ),
      htaccess = COALESCE(
        CASE WHEN p_data ? 'htaccess' AND p_data->>'htaccess' IS NOT NULL THEN NULLIF(p_data->>'htaccess', '') END,
        htaccess
      ),
      redirect_404 = COALESCE(
        CASE WHEN p_data ? 'redirect_404' AND p_data->>'redirect_404' IS NOT NULL THEN (p_data->>'redirect_404')::boolean END,
        redirect_404
      )
    WHERE uuid = v_uuid
    RETURNING * INTO v_result;
  ...
  -- History insertion is handled by trigger trg_log_website_history
