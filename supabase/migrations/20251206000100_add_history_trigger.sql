-- Create a function to handle history logging automatically on any update
CREATE OR REPLACE FUNCTION public.handle_website_history()
RETURNS TRIGGER AS $$
BEGIN
  -- Check if the only change is the status field
  -- We compare the full row converted to jsonb, but we remove 'status' and 'updated_at' from comparison
  -- updated_at always changes on update, so we must ignore it too
  IF (to_jsonb(NEW) - 'status' - 'updated_at') = (to_jsonb(OLD) - 'status' - 'updated_at') THEN
    RETURN NEW;
  END IF;

  -- Insert the new state into history
...
-- Create the trigger to run after any update
DROP TRIGGER IF EXISTS trg_log_website_history ON public.websites;
CREATE TRIGGER trg_log_website_history
AFTER UPDATE ON public.websites
FOR EACH ROW
EXECUTE FUNCTION public.handle_website_history();

-- Update fn_upsert_website to REMOVE manual history insertion (to avoid duplicates with the trigger)
...
BEGIN
  ...
  -- History insertion removed here as it is now handled by the trigger trg_log_website_history
